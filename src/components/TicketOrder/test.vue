<template>
    <popup v-model="showStaff" position="right" width="100%">
        <div class="Entstaff">
                ...
                <ul class="EntsChoice" >
                    <li v-for="(choice,i) in staff.staffList" :key="i">
                            ...
                            <label class="check" @click="checked(choice, i)" :class="{checked:choice.isChecked}"></label>
                    </li>
                </ul>
                <div class="EntsCfid">
                    <div class="entsdiv" @click="operPopup">已选择：{{filterChosedEnts.length}}人</div>
                    <div @click="sureEntsPerson">确定</div>
                </div>
                <popup v-model="showPopup" position="bottom">
                    <ul class="chosedPersonList">
                            <li class="chosedPersonItem" v-for="(item, index) in filterChosedEnts" :key="index">
                                {{item.PsgerName}}
...
export default {
    ...
    computed: {
        ...
        filterChosedEnts() {
            this.chosedEnts = this.staff.staffList.filter(item => {
                return item.isChecked
            })
            return this.chosedEnts
        },
    },
    created() {
        let staffParams = {
            url: 'MyFrequentPassenger/GetFrequentPsger',
            params: {
        ...
        this.$store.dispatch('getFrequentPsger', staffParams) 
    },
    methods: {
        goBack(){
            this.showPopup = false
            this.$emit('close-staff', this.chosedEnts)
        },
        checked(item, index){
        ...
        deleteData(item, index) {
                ...
                this.staff.staffList.map((val, i) => {
